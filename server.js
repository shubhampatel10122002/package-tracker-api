...
(async () => {
  try {
    await fs.access(COOKIES_PATH).catch(() => {
      console.log('Creating empty cookies file');
      return fs.writeFile(COOKIES_PATH, JSON.stringify([]));
    });
  } catch (error) {
    console.error('Error creating/checking cookies file:', error.message);
  }
})();
...
app.post('/api/track', async (req, res) => {
  ...
  try {
    ...
    // Try to load cookies if they exist (for historical compatibility)
    let cookies = [];
    try {
    ...
    let result;
    let newCookies = []; // Initialize empty array for cookies

    if (USE_FALLBACK) {
      // Use the fallback tracker
      console.log('Using fallback tracker (simple implementation)');
      const fallbackTracker = new FallbackTracker();
      result = await fallbackTracker.track(trackingNumber, courier);
    } else {
      // Use the full Puppeteer tracker with proxy
      console.log('Using Puppeteer tracker with proxy support');
      ...
      // Store any new cookies for future compatibility
      if (tracker.newCookies && tracker.newCookies.length > 0) {
        newCookies = tracker.newCookies;
...
app.listen(PORT, () => {
  ...
  console.log(`- Using proxy: ${process.env.USE_PROXY === 'true'}`);
  console.log(`- Using fallback: ${USE_FALLBACK}`);
});
...
//direct version that works well and just needs to be adjusted for ip
// const express = require('express');
// const bodyParser = require('body-parser');
// const cors = require('cors');
// const { PackageRadarTracker } = require('./PackageRadarTracker');
