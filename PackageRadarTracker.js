...
class PackageRadarTracker {
  constructor(options = {}) {
    this.options = {
      ...
      // Proxy settings
      useProxy: options.useProxy !== false, // Default to using proxy if available
      proxyHost: options.proxyHost || process.env.PROXY_HOST || 'gate.smartproxy.com',
      proxyPort: options.proxyPort || process.env.PROXY_PORT || '10001',
      proxyUser: options.proxyUser || process.env.PROXY_USER || 'spjmpkfax7',
      proxyPass: options.proxyPass || process.env.PROXY_PASS || 'r0grT~w4Lg0ykdIV4q'
    };
  ...
  async track(trackingNumber, courier = 'ups') {
    ...
    while (retries <= this.options.maxRetries) {
      try {
        // Construct proxy URL if proxy is enabled
        const proxyUrl = this.options.useProxy 
          ? `http://${this.options.proxyUser}:${this.options.proxyPass}@${this.options.proxyHost}:${this.options.proxyPort}`
          : null;
        ...
        const launchOptions = {
          ...
          args: [
            ...
            ...(proxyUrl ? [`--proxy-server=${this.options.proxyHost}:${this.options.proxyPort}`] : [])
          ]
        };

        console.log('Launching browser...');
        if (proxyUrl) {
          console.log(`Using proxy: ${this.options.proxyHost}:${this.options.proxyPort}`);
        }
        ...
        if (proxyUrl && this.options.proxyUser && this.options.proxyPass) {
        ...
        // Basic browser fingerprinting evasion
        ...
        // Verify proxy IP (optional, for debugging)
        if (this.options.useProxy && this.options.saveScreenshot) {
          try {
            await page.goto('https://ip.smartproxy.com/json', {
              waitUntil: 'networkidle2',
              timeout: 10000
            });
            await page.screenshot({ path: 'proxy-ip-check.png' });
            console.log('Proxy IP check screenshot saved');
          } catch (err) {
            console.log('IP check skipped:', err.message);
          }
        }

        // Navigate to the tracking URL directly (no complex Cloudflare handling needed with proxy)
        ...
        // Try multiple selector patterns to find tracking info
        const selectors = [
        ...
        if (this.options.saveScreenshot) {
          await page.screenshot({ path: 'packageradar-page.png', fullPage: true });
        ...
        // Extract the tracking information using the same logic as before
        const result = await page.evaluate(() => {
          ...
          // Process checkpoints (using the same logic as your original code)
